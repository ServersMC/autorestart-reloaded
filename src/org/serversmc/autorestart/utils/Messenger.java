...
public class Messenger {
    ...
    public static void broadcastMinutes(Integer m) {
        if (Config.BROADCAST.ENABLED.MINUTES()) {
            ...
            Bukkit.broadcastMessage(ChatColor.translateAlternateColorCodes('&', prefix + Config.BROADCAST.MESSAGES.MINUTES().replaceAll("%m", m.toString())));
        }
    }
    ...
    public static void broadcastSeconds(Integer s) {
        if (Config.BROADCAST.ENABLED.SECONDS()) {
            String prefix = Config.BROADCAST.MESSAGES.PREFIX();
            Bukkit.broadcastMessage(ChatColor.translateAlternateColorCodes('&', prefix + Config.BROADCAST.MESSAGES.SECONDS().replaceAll("%s", s.toString())));
        }
    }
    ...
    public static void broadcastChange(Integer h, Integer m, Integer s) {
        if (Config.BROADCAST.ENABLED.CHANGE()) {
            String prefix = Config.BROADCAST.MESSAGES.PREFIX();
            Bukkit.broadcastMessage(ChatColor.translateAlternateColorCodes('&', prefix + Config.BROADCAST.MESSAGES.CHANGE().replaceAll("%h", h.toString()).replaceAll("%m", m.toString()).replaceAll("%s", s.toString())));
        }
    }
